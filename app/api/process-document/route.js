...
const processDocumentInBackground = async ({
  ...
  // Temporarily skip auto-summarization due to HuggingFace API issues
  // Use filename as title instead
  const title = fileName.replace('.pdf', '').replace(/[^a-zA-Z0-9\s]/g, ' ').trim();
